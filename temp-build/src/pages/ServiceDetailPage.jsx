...
const ServiceDetailPage = () => {
  ...
  const benefitsList = useMemo(() => {
    if (!service?.beneficios) return [];
    const sep = service.beneficios.includes('\n') ? '\n' : ',';
    return service.beneficios.split(sep).map(b => b.trim()).filter(Boolean);
  }, [service]);

  const whatsappUrl = useMemo(() => {
    const num = contact?.whatsapp ?? '62981070937';
    const msg = contact?.mensagem_whatsapp
      ?? `Olá! Gostaria de mais informações sobre ${service?.nome ?? 'o serviço'}.`;
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <SEO
        title={`${service.nome} | Instituto Milhomem`}
          ...
          <div className="container-custom relative z-10 py-24 mt-16">
            ...
            >
              ...
              <div className="flex items-center gap-2 text-white/50 text-xs font-bold uppercase tracking-widest mb-6">
                ...
                <span>/</span>
                <span className="text-primary">{service.nome}</span>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
              ...
              >
                {service.conteudo ? (
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: service.conteudo }}
                  />
                ) : (
              ...
              >
                ...
                <div className="bg-secondary rounded-2xl p-6 border border-primary/20 text-center space-y-4">
                  <PhosphorIcons.WhatsappLogo size={32} weight="fill" className="text-primary mx-auto" />
                  <h3 className="font-bold text-white text-base">{pageConfig.sidebar_title}</h3>
                  <p className="text-white/70 text-sm leading-relaxed">
                    {pageConfig.sidebar_text_prefix} {service.nome}.
                  </p>
                  <a
                    href={whatsappUrl}
                    target="_blank" rel="noopener noreferrer"
