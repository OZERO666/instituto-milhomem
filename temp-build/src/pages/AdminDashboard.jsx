...
import {
  ...
  SeoTab, PagesTab, SobreTab, HeroTab,
...
const AdminDashboard = () => {
  ...
  const anyDirty = [
    ...
    testimonialForm, statsForm, contactForm,
  ...
  const fetchData = useCallback(async () => {
    try {
      switch (activeTab) {
        ...
        case 'overview':     return Promise.all([
          fetchBookings(), fetchServicos(), fetchGaleria(), fetchBlog(),
          fetchDepoimentos(), fetchEstatisticas(), fetchContato(), fetchAuditLogs(),
        ]);
        default: return;
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-muted flex flex-col">
        ...
        <div className="container-custom py-8 flex-grow flex flex-col">
          <Tabs value={activeTab} onValueChange={(tab) => guardTab(tab, setActiveTab)} className="flex-grow flex flex-col">
            <AdminTabsNav unreadBookings={bookings.filter((b) => !b.lido).length} />
            ...
            <TabsContent value="contact">
              <ContatoTab
                contactForm={contactForm} isLoading={loadingContato}
